...
- **LDAP statt PowerShell/RSAT:** `Novell.Directory.Ldap.NETStandard` ersetzt das Windows-only `ActiveDirectory`-Modul und umgeht bekannte StartTLS-Probleme von `System.DirectoryServices.Protocols` unter Linux.
...
- Der verwaltete LDAP-Client prueft Zertifikate selbst und benoetigt im Container weder `libldap` noch `TLS_REQCERT`.
...
- Der Managed-LDAP-Transport wurde in zwei isolierten Docker-Containern gegen einen echten LDAP-Server mit selbstsigniertem Zertifikat geprueft.
- Gepruefte Kombination wie im Ticketsystem: LDAP-Port `389`, LDAPS aus, StartTLS an, Zertifikatspruefung aus.
- Erfolgreich waren Verbindung, StartTLS, Bind, SearchBase, Gruppensuche ohne Paging, Gruppensuche mit Paging und der normale `/api/search`-Pfad.
